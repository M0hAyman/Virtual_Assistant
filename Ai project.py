...
def main():
    ...
    #call(["python","sound_visualizer.py"])
...
#    #gui 
   ...
   Mic_btn=tk.Button(button_frame,text='')
   icon= PhotoImage(file='icon2-13.png')
   root.iconphoto(True,icon)
   root.config(background='black')
   button_photo=Image.open("Mic.png")
   resized=button_photo.resize((55,55),Image.ANTIALIAS)
   new_ButtonPhoto=ImageTk.PhotoImage(resized)
   listen_button=tk.Button(root,image=new_ButtonPhoto,borderwidth=0,activebackground='white',bg='black',command= main)
   listen_button.place(x=330,y=150)
   #text_label = Label(root,"HEllo",bg="blcak",fg="FFFFFF",padx=100,pady=200)
   #text_label.pack()
